...
const userSocketMap = {};
const notebookContent = require('./utils/notebookContent.json') || {};
...
const getPendingMessagesInRooms = ({ rooms, role }) => {
  ...
  rooms.map((room) => {
    keys.map((key) => {
      ...
      if (
        room === r &&
        notebookContent[key][`${role}PendingMessages`].length > 0
      ) {
...
io.on(CONNECTION, (socket) => {
  ...
  socket.on(JOIN, async ({ notebook, user_id }) => {
    ...
    if (!notebookContent[notebook]) {
      ...
      console.log("creating new notebook", notebookContent[notebook]);
    }

    if (role) {
      console.log("setting", role, "in", notebook, "as active");
  ...
  socket.on(LEAVE, ({ notebook, user_id }) => {
    ...
    if (role) {
      console.log("setting", role, "in", notebook, "as inactive");
